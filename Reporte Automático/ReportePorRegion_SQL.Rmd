...
```{r Cargar Datos}
source("Scripts/CargaDatos_SQL.R", encoding = "UTF-8")
```
...
###### A continuación, se presenta un ejemplo de minuta con diferentes indicadores asociados a la **`r reg`**. Los datos son simulados y no deben ser considerados en ningún caso como información oficial de la SMA. Este reporte fue generado el **`r format(Sys.Date(), "%d/%m/%Y")`**. 
